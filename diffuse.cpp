...
# pragma omp parallel for shared(f_tmp)
  ...
  while(t<m_Tfin){
	...
	break;
      case 2:
	u_y=m_M->getBinfCell_x(i)+0.5*m_M->getDxCell(i);
	u_x=-(m_M->getBinfCell_y(i)+0.5*m_M->getDyCell(i));
	break;
      default:
	...
	break;
      ...
      x=i%m_nbCellx;
      y=i/m_nbCellx;
      f_tmp.at(i)=m_M->getValueCell(i)-dt*0.5*psix(x,y,u_x)/m_M->getDxCell(i);
	-dt*0.5*psiy(x,y,u_y)/m_M->getDyCell(i);
    } 
...
# pragma omp parallel for shared(f_tmp) private(u_x,u_y,x,y)
	...
	break;
      ...
      x=i%m_nbCellx;
      y=i/m_nbCellx;
      f_tmp.at(i)=f_tmp2.at(i)-dt*psix(x,y,u_x)/m_M->getDxCell(i)
	-dt*psiy(x,y,u_y)/m_M->getDyCell(i);
    } 
# pragma omp parallel for shared(f_tmp) private(Val)
    for(int i=0;i<m_M->getNbCell();i++){
      Val=f_tmp.at(i);
...
double Diffuse:: phi_demipp(int i,int j){
  double phi;
  double tmp;
  double grad_tmp;
  double zsup,zsup_tmp;
  tmp=0;
  grad_tmp=gradient_x(i,j)*m_M->getDxCell(0)*0.5+gradient_y(i,j)*m_M->getDyCell(0)*0.5;
  if(grad_tmp>0)
    {
...
double Diffuse:: phi_demipm(int i,int j){
  double phi;
  double tmp;
  double grad_tmp;
  double zsup,zsup_tmp;
...
double Diffuse:: phi_demimm(int i,int j){
  double phi;
  double tmp;
  double grad_tmp;
  double zsup,zsup_tmp;
...
double Diffuse:: phi_demimp(int i,int j){
  double phi;
  double tmp;
  double grad_tmp;
  double zsup,zsup_tmp;
...
double Diffuse:: phi(int i , int j){
  double phi;
  double tmp;
  ...
  tmp=std::min(phi_demipp(i,j),phi_demimp(i,j));
  phi=std::min(phi_demimm(i,j),phi_demipm(i,j));
  phi=std::min(phi,tmp);;
  return phi;
...
double Diffuse::z_plus(int i , int j,int coor){ /// rassembler zplus et zmoins pour 1 seul calcul de gradient 
  double z;
  if(coor==0){
    z=m_M->getValxy(i,j)-phi(i,j)*(gradient_x(i,j)*m_M->getDyCell(0)*0.5);
  }
  else
    {
      z=m_M->getValxy(i,j)-phi(i,j)*(gradient_y(i,j)*m_M->getDyCell(0)*0.5);
    }
  return z;  
}
